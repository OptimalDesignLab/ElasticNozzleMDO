...
class ElasticNozzleIDF(UserSolverIDF):
    ...
    def __init__(self, ndv, nodes):
        super(ElasticNozzleIDF, self).__init__(
            ndv, 6*nodes, 2*nodes,
            num_eq=0, num_ineq=0)
    ...
    def eval_eq_cnstr(self, at_design, at_state):
        mda.set_design_data(0, at_design)
        mda.eval_c(0, at_state.idx, 0)
        return mda.get_dual_data(0)
    ...
    def apply_precond(self, at_design, at_state, in_vec, out_vec):
        ...
        return mda.apply_precond(
            0, at_state.idx, in_vec.idx, out_vec.idx)
